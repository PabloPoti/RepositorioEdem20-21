# -*- coding: utf-8 -*-
"""
Created on Fri Dec 17 15:56:51 2021
...
model6 = ols('cnt ~ temp_celsius + windspeed_kh + hum + workingday + yr + cloudy + rainy'
             , data=wbr).fit() #SE DEBE DEJAR UNA DE LAS 3 VBLES. DUMMIES FUERA (lo más común fuera, la categoría modal --> ayuda a la interpretación)
...
plt.scatter(wbr.cnt, wbr.goal) #--> Para el QC OK

wbr.goal.hist()

#Importamos librería para poder hacer regresiones con la variable dependiente cualitativa
...
model_l7 = logit('goal ~ temp_celsius + windspeed_kh + hum + workingday + yr + cloudy + rainy', data=wbr).fit()
print(model_l7.summary2())


wbr['temp2'] = wbr.temp_celsius**2 # Crear una nueva vble, temperatura al cuadrado

#OLS --> REGRESIÓN LINEAL CUADRÁTICA

model8 = ols('cnt ~ temp_celsius + windspeed_kh + hum + workingday + yr + cloudy + rainy', data=wbr).fit()
print(model_l1.summary2())
